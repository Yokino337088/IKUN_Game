...
- **Inspector 快捷入口**：AnimationClip、材质纹理、Prefab 等资产 Inspector 标题栏可直接打开对应 AI 生成窗口
...
3. 输入文本提示词或上传参考图片（**3D 模型窗口内切换生成器时，文本提示词在各模型间共享**；切到不需要文本输入的生成器时会自动清空）
4. 调整参数（可选）
5. 点击 **生成** 按钮开始生成
...
| 包 | 版本 | 用途 |
|----|------|------|
| `cn.tuanjie.codely.bridge` | 1.0.34 | Codely 桥接 |
