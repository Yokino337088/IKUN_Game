...
namespace TJGenerators
{
    ...
    public class TJGeneratorsImageWindow : GenerationWindowBase, IGenerationPipelineHost
    {
        ...
        private void DrawFrontierSequenceProfileSelector()
        {
            ...
            GUILayout.Space(10);
            GUILayout.Label("网格规格：", CommonStyles.HeaderStyle);
        ...
        private void DrawSpriteCutoutAndSliceContent()
        {
            ...
            GUILayout.Label(
                "在右侧预览区选中历史图片后，可抠图、切割并导出 Sprite 动画；也可直接导入本地图片。",
                CommonStyles.HelpBoxStyle,
                GUILayout.MaxWidth(CommonStyles.LeftComponentWidth));
            GUILayout.Space(CommonStyles.Space2);

            GUILayout.BeginHorizontal();
            if (GUILayout.Button("导入本地图片…", GUILayout.Height(22), GUILayout.ExpandWidth(true)))
                ImportLocalImageToHistory();
            GUILayout.Space(CommonStyles.Space2);
            if (GUILayout.Button("使用当前选中图片", GUILayout.Height(22), GUILayout.ExpandWidth(true)))
            ...
            {
                GUILayout.Label(
                    "未找到可处理的图片：请先在右侧历史中选一张，或点击上方「导入本地图片 / 使用当前选中图片」。",
                    CommonStyles.HelpBoxStyle,
                    GUILayout.MaxWidth(CommonStyles.LeftComponentWidth));
                return;
            }
            ...
            chromaKeyTolerance = UIComponents.DrawAdvancedStyleSliderRow(
                "绿幕容差", chromaKeyTolerance, 0.05f, 0.35f);
            GUILayout.Space(CommonStyles.Space2);
            chromaFeather = UIComponents.DrawAdvancedStyleSliderRow(
                "边缘羽化", chromaFeather, 0f, 0.3f);

            GUILayout.Space(CommonStyles.Space2);
            GUILayout.BeginHorizontal();
            if (GUILayout.Button("执行抠图并预览", GUILayout.Height(24), GUILayout.ExpandWidth(true)))
                ApplyGreenScreenCutoutPreview();
            GUILayout.Space(CommonStyles.Space2);
            if (GUILayout.Button("恢复原图预览", GUILayout.Height(24), GUILayout.ExpandWidth(true)))
            {
                ResetProcessedPreview();
                Repaint();
            }
            ...
            spriteSliceColumns = Mathf.Max(
                1,
                UIComponents.DrawAdvancedStyleIntRow("切割列数", spriteSliceColumns, "sprite_slice_columns"));
            GUILayout.Space(CommonStyles.Space2);
            spriteSliceRows = Mathf.Max(
                1,
                UIComponents.DrawAdvancedStyleIntRow("切割行数", spriteSliceRows, "sprite_slice_rows"));
            GUILayout.Space(CommonStyles.Space2);
            spriteSliceFps = UIComponents.DrawAdvancedStyleSliderRow("动画 FPS", spriteSliceFps, 1f, 60f);
            GUILayout.Space(CommonStyles.Space2);
            spriteSliceLoop = UIComponents.DrawAdvancedStyleBoolRow("动画循环 (Loop)", spriteSliceLoop);
            GUILayout.Space(CommonStyles.Space2);
            GUILayout.Label(
                "预览图中的红色线条为切割线。",
                CommonStyles.HelpBoxStyle,
            ...
            GUILayout.Space(CommonStyles.Space2);
            if (GUILayout.Button("切割并导出为 Sprite", GUILayout.Height(28), GUILayout.ExpandWidth(true)))
        ...
        private void ApplyHistoryToImage(int index)
        {
            ...
            if (
                !EditorUtility.DisplayDialog(
                    "确认替换",
                    $"确定将选中的图片应用到当前目标「{Path.GetFileNameWithoutExtension(targetPathForDialog)}」吗？",
                    "确定",
                    "取消"
                )
            )
            ...
            if (!ReplaceTargetImageFromSource(item.modelPath, "已将历史图片应用到", out string err))
        ...
        private bool ReplaceTargetImageFromSource(string sourceAssetPath, string okLogVerb, out string errorMessage)
        {
            ...
            try
            {
                ...
                if (!string.Equals(originalPath, targetPath, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    titleContent = new GUIContent($"TJGenerators 图片 - {Path.GetFileNameWithoutExtension(targetPath)}");
        ...
        public void OnTextureSaved(string savePath, ModelGeneratorBase generator)
        {
            ...
            if (!ReplaceTargetImageFromSource(savePath, "已生成图片并复制到", out string replaceErr))
