...
namespace TangmenFramework
{
    ...
    public class GOPoolMgr : BaseManager<GOPoolMgr>
    {
        ...
        public async UniTask<GameObject> GetObj(string abName, string resName)
        {
            //如果池根对象为空且开启了布局，则创建
            if (poolObj == null && isOpenLayout)
                poolObj = new GameObject("Pool");
                ...
                (poolDic[abName + resName].Count == 0 && poolDic[abName + resName].NeedCreate))
            {
                //动态创建对象
                //没有对象时，通过资源加载来实例化一个GameObject            
                obj = GameObject.Instantiate(await ABResMgr.Instance.LoadResAsync<GameObject>(abName, resName));
