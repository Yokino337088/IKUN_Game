...
namespace TangmenFramework
{
    public class UWQResMgr : SingletonAutoMono<UWQResMgr>
    {
        ...
        public void LoadRes<T>(string path, Action<T> callBack, Action failCallBack) where T : class
        {
            StartCoroutine(ReallyLoadRes<T>(path, callBack, failCallBack));
        }

        private IEnumerator ReallyLoadRes<T>(string path, Action<T> callBack, Action failCallBack) where T : class
        {
            //string
            //byte[]
            //Texture
