...
namespace TangmenFramework
{
    ...
    public abstract class BasePanel : MonoBehaviour
    {
        ...
        protected virtual void DropDownSelectChange(string dropDownName, int index)
        {
            ...
            for (int i = 0; i < controls.Length; i++)
            {
                ...
                if (!controlDic.ContainsKey(controlName))
                {
                    if (!defaultNameList.Contains(controlName))
                    {
                        ...
                        {
                            (controls[i] as Dropdown).onValueChanged.AddListener((index) =>
                            {
                                DropDownSelectChange(controlName, index);
                            });
                        }
                        else if (controls[i] is TMP_Dropdown)
                        {
                            (controls[i] as TMP_Dropdown).onValueChanged.AddListener((index) =>
                            {
                                DropDownSelectChange(controlName, index);
                            });
                        }
