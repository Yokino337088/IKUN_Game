...
public class BossController : MonoBehaviour, IBossFSMObj
{
    ...
    /// 原缩放
    /// </summary>
    private Vector3 originalScale = Vector3.one * 6f;
    ...
    public float laserOrbitRadius = 3f;
    ...
    public Transform 伤害生成点左;
    public Transform 伤害生成点右;
    ...
    /// 弹幕工厂的实例
    /// </summary>
    BulletFactory IBossFSMObj.BulletFactory => bulletFactory;

    /// <summary>
    /// 显示伤害字幕
    /// </summary>
    /// <param name="damage"></param>
    private void CreateDamageText(DamageDisplayInfo info)
    {        
        DamageText damageText = GOPoolMgr.Instance.GetObj(DamageText).GetComponent<DamageText>();
        damageText.Init(info);
        Vector3 dir = playerTransform.position - transform.position;
        damageText.transform.position = dir.x > 0 ? 伤害生成点右.position : 伤害生成点左.position;
    }

    void IBossFSMObj.SpawnLasers()
    {
        //计算角度偏移，使激光均匀分布在圆周上
        float angleStep = Mathf.PI * 2f / laserCount;
        for (int i = 0; i < laserCount; i++)
        {
            float angle = angleStep * i;
            //调用工厂方法生成激光，传入Boss的Transform、半径、角度和旋转速度
            BossLaser laser = bulletFactory.SpawnLaser(transform, laserOrbitRadius, angle, laserRotationSpeed);
            if (laser != null)
            {
                //将激光的GameObject添加到当前存活的激光列表中，方便后续管理和回收
                activeLasers.Add(laser.gameObject);
            }
        }
    }

    void IBossFSMObj.RecycleAllLasers()
    {
        foreach (var laserGO in activeLasers)
        {
            if (laserGO != null)
            {
                ...
                if (laser != null)
                    ...
                    GOPoolMgr.Instance.PushObj(laserGO);
            }
        }
        activeLasers.Clear();
    }

    void IBossFSMObj.SpawnFireBall()
    {
        // 大火球初始方向：指向玩家
        Vector2 direction = (playerTransform.position - transform.position).normalized;
        bulletFactory.SpawnBullet3(transform.position, direction, 10f);
    }
    ...
    private void Awake()
    {
        ...
        bulletFactory.Init(bulletPrefab1, bulletPrefab2, bulletPrefab3, laserPrefab);
        // 记录初始位置
    ...
    private void Start()
    {
        ...
        BossLevelMgr.Instance.RegisterBoss(this);
        // 初始化状态机
        InitializeStateMachine();
        // 开始浮空动画
        StartFloatingAnimation();

        EventCenter.Instance.AddEventListener<DamageDisplayInfo>(MyEventTypeString.玩家伤害显示事件, CreateDamageText);
    }
    ...
    private void Update()
    {
        ...
        NowHp = bossData.NowHp;
        NowPhase = bossData.nowPhase;
    }
    ...
    private void OnDestroy()
    {
        // 清理动画
        StopAllTweens();
        EventCenter.Instance.RemoveEventListener<DamageDisplayInfo>(MyEventTypeString.玩家伤害显示事件, CreateDamageText);
    }
    ...
    public void MoveTo(Vector3 target, float duration = 0.5f, Action onComplete = null)
    {
        ...
        bool wasFloating = floatTween != null && floatTween.IsActive();
        if (wasFloating)
        {
            floatTween.Pause();
        }
        ...
        currentMoveTween = transform.DOMove(target, duration)
            ...
            {
                IsMoving = false;
                if (wasFloating && floatTween != null)
                {
                    floatTween.Play();
                }
                onComplete?.Invoke();
    ...
    {
        LogSystem.Debug($"获取动画剪辑：Boss阶段={bossPhaseType}");
        
        if (clipDictionary.TryGetValue(bossPhaseType, out AnimationClip clip))
        {
            return clip;
        }
        
        LogSystem.Error($"找不到阶段 {bossPhaseType} 的动画剪辑！");
        return null;
    }
    ...
    void IBossFSMObj.DoAttackStartAnimation()
    {
        ...
        sequence.Append(transform.DOScale(originalScale * 1.3f, 0.3f).SetEase(Ease.OutQuad));
        sequence.Append(transform.DOScale(originalScale, 0.2f).SetEase(Ease.InQuad));
    ...
    {
        float horizontalDir = Mathf.Sign(playerTransform.position.x - transform.position.x);
        Vector3 targetPos = transform.position + new Vector3(horizontalDir * 10f, 0, 0);

        currentMoveTween = transform.DOMove(targetPos, 1f) 
            .SetEase(Ease.Linear)
    ...
    {
        MoveTo(boss原点.position, 1f, onComplete);
    }
    ...
    void IBossFSMObj.DoChargeAnimationEffect(float duration)
    {
        ...
        chargeEffectTween = transform.DOScale(7f, duration)
    ...
    /// </summary>
    /// <param name="target">目标世界坐标</param>
    void IBossFSMObj.TeleportTo(Vector3 target, Action callBack)
    {
        //先停止当前的移动
        StopCurrentMove();
    ...
    {
        if (collision.CompareTag("玩家子弹"))
        {
            if (collision.TryGetComponent<BossPlayerBullet>(out var playerBullet))
            {
                MusicMgr.Instance.PlaySoundSafe(MyAssetBundleName.通用音效包, "受击");

                // 计算伤害并应用到Boss数据
                int dmg = playerBullet.CalculateDamage(out bool isCrit);
                bossData.GetDamage(dmg, isCrit);

                // 回收子弹
                playerBullet.Recycle();
            }
        }
        else if (collision.CompareTag("大招弹幕"))
        {
            if (collision.TryGetComponent<BulletComment>(out var bulletComment))
            {
                MusicMgr.Instance.PlaySoundSafe(MyAssetBundleName.通用音效包, "受击");
                // 计算伤害并应用到Boss数据
                int dmg = bulletComment.CalculateDamage(out bool isCrit);
                bossData.GetDamage(dmg, isCrit);
            }
        }
    }

    void IBossFSMObj.PlayTeleportArrivalEffect()
    {
        // 从对象池获取瞬移特效对象，设置位置并激活
        GameObject eff = GOPoolMgr.Instance.GetObj(teleportEffect);
